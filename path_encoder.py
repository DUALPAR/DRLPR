...
def p_embed_seq(input_seq, from_, to_, is_training, BN=True, initializer=tf.contrib.layers.xavier_initializer()):
    with tf.variable_scope("p_embedding",reuse=None):  # embed + BN input set
        #tf.variable_scope("weights",reuse=tf.AUTO_REUSE)
        W_embed = tf.get_variable("p_weights", [1, from_, to_], initializer=initializer)
        #tf.variable_scope("weights",reuse=tf.AUTO_REUSE)
        embedded_input = tf.nn.conv1d(input_seq, W_embed, 1, "VALID", name="p_embedded_input")
        if BN:
            embedded_input = tf.layers.batch_normalization(embedded_input, axis=2, training=True, name='p_layer_norm', reuse=None)
...
def p_multihead_attention(inputs, num_units=None, num_heads=16, dropout_rate=0.1, is_training=True):
	with tf.variable_scope("p_multihead_attention", reuse=None):
		# Linear projections
...
def p_feedforward(inputs, num_units=[2048, 512], is_training=True):
	with tf.variable_scope("p_ffn", reuse=None):
		# Inner layer
		...
		outputs += inputs
		# Normalize
		outputs = tf.layers.batch_normalization(outputs, axis=2, training=True, name='p_ln', reuse=None)  # [batch_size, seq_length, n_hidden]
	return outputs


# Encode input sequence [batch_size, seq_length, n_hidden] -> [batch_size, seq_length, n_hidden]
def p_encode_seq(input_seq, input_dim, num_stacks, num_heads, num_neurons, is_training, dropout_rate=0.):
	with tf.variable_scope("p_stack"):
		for i in range(num_stacks):  # block i
			with tf.variable_scope("p_block_{}".format(i)):  # Multihead Attention + Feed Forward
				input_seq = p_multihead_attention(input_seq, num_units=input_dim, num_heads=num_heads, dropout_rate=dropout_rate, is_training=is_training)
				input_seq = p_feedforward(input_seq, num_units=[num_neurons, input_dim], is_training=is_training)
